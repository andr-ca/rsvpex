...
rsvpSubmitRouter.post('/:slug', turnstileVerify(), rsvpRateLimit(), async (c) => {
  ...
  if (!email && !phone) {
    return c.json(
      { error: 'validation_failed', issues: [{ message: 'Email or phone is required' }] },
      400,
    )
  }
  ...
  if (event.enable_heuristic_dup_check) {
    const heuristic = await isHeuristicDuplicate(
      c.env.DB,
      event.id,
      body.name,
      email,
